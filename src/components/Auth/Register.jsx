...
export const Register = ({ onSwitch }) => {
    ...
    const validateStep = () => {
        if (step === 1) {
            return formData.nombre && formData.apellido && formData.apodo && formData.phone;
        }
        if (step === 2) {
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            return formData.address && formData.correo && emailRegex.test(formData.correo) && formData.password;
        }
        if (step === 3) {
                ...
                <div className="w-full rounded-xl sm:max-w-md xl:p-0 bg-stone-50">
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                            {step === 1 && (
                                <>
                                    <Input name="nombre" label="Nombre" value={formData.nombre} onChange={handleChange} placeholder="Juan" />
                                    <Input name="apellido" label="Apellido" value={formData.apellido} onChange={handleChange} placeholder="Pérez" />
                            ...
                            {step === 2 && (
                                <>
                                    ...
                                    <Input name="correo" label="Correo electrónico" type="email" value={formData.correo} onChange={handleChange} placeholder="example@domain.com" />
