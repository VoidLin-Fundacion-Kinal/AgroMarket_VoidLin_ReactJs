...
import { X, Package, FileText, DollarSign, Weight, Check } from "lucide-react"

const UpdateProductModal = ({ showModal, setShowModal, setProducts, productToEdit }) => {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      setLoading(true)
      const data = await updateProduct(productToEdit._id, formData)
      
      if (data.success) {
        Swal.fire("Producto actualizado", "El producto se ha actualizado correctamente", "success")
        setShowModal(false)
        setProducts(prev => prev.map(p => p._id === productToEdit._id ? { ...p, ...formData } : p))
      } else {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden transform animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm">Actualizar información básica del producto</p>
              </div>
            </div>
            <button
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          <form onSubmit={handleUpdateProduct} className="space-y-6">
            ...
            {/* Campo Peso */}
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-gray-700">Peso</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
